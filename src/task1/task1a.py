...
if __name__ == '__main__':
    ...
    for j in range(N_runs):
        ...
        y_train = polynomial_fun(w, x_train).to(device)
        y_test = polynomial_fun(w, x_test).to(device)
        t_train = y_train + 0.5 * torch.randn_like(y_train).to(device)
        t_test = y_test + 0.5 * torch.randn_like(y_test).to(device)
