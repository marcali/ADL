...
def fit_polynomial_sgd(x, t, M = 0, reg_param = 0.01, lr=1e-2, miniBatchSize=5, N_epochs=2400, ):
    ...
    for epoch in range(N_epochs):
        for batch_x, batch_t in loader:
            ...
            loss = mse_loss(batch_y, batch_t) + reg_param * torch.norm(weights,1)
            loss.backward() 
...
if __name__ == '__main__':
    with open('output_task1a.txt', 'w') as f:
        ...
        for j in range(N_runs):
            ...
            for i, m in enumerate(m_values):
                for k, reg in enumerate(lambda_values):
                    ...
                    #for SGD
                    w_hat_opimized = fit_polynomial_sgd(x_train, t_train, m, reg)
                    #training
                    pred_train_opt = polynomial_fun(w_hat_opimized, x_train).squeeze()
                    
                    #rmse train
                    diff_pred_train_opt = pred_train_opt - t_train
                    diff_pred_weights_opt = w_true.view(-1, 1)- w_hat_opimized
        ...
        w_hat_opimized_test = fit_polynomial_sgd(x_test, t_test, best_m, best_reg)
        pred_test_opt = polynomial_fun(w_hat_opimized_test, x_test).squeeze()
        diff_pred_weights_opt = w.view(-1, 1) - w_hat_opimized_test
